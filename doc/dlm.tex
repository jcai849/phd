...
is fitted using \pkg{biglm} as part of a demonstrative \hsrc{Reduce()} in listing \ref{lst:lm-reduce.R}.\src[caption={Splitting the iris dataframe into 15 chunks stored as elements of a list and reducing over the list with the biglm update function.}]{lm-reduce.R}
...
Its form is given in the diagram at Figure \ref{fig:ins-env}.\img[caption={Newly inserted environment mimicking the isertion procedure of a linked list with the old pointer in dashed line.}]{ins-env}
