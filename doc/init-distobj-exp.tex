...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\node[queue,right={3*\blockdist} of cmdotq] (cmdotqdot) {\(\dots\)};
		\node[queue,right={3*\blockdist} of cmq] (cmqdot) {\(\dots\)};
		...
		\begin{pgfonlayer}{units}
			\node[unit,fit=(dnl)(dil)(dor)(sendreq),label={[name=cl] Client}] (c) {};
